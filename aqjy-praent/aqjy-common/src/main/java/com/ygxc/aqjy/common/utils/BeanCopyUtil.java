...
public class BeanCopyUtil implements Serializable {	
    ...
    public static void copyProperties(Object to, Object from) {
        try {
        	...
        	throw new YgxcBusinessException("Bean copy error!", e);
        }
    }

    @SuppressWarnings("unchecked")
	public static <T> List<T> copyCollection(List<T> collection, Class<?> aclass) {
	...
	public static Map<?, ?> describe(Object from) {
        try {
        	...
        	throw new YgxcBusinessException("Bean copy describe error!", e);
        }
    }
     ...
     */
    @SuppressWarnings("unchecked")
	public static <T> List<T> getBatchObjectFromParam(Map<?, ?> params, Class aclass, int paramNum) {
    ...
    /**
     * 复制属性，如果属性不为null才进行复制
     * @param to
     * @param from
     */
    public static void copyRealProperties(Object to, Object from) {
    	...
    	try {
    		...
    		PropertyDescriptor[] toPdArr = org.springframework.beans.BeanUtils.getPropertyDescriptors(toClass);
    		PropertyDescriptor[] fromPdArr = org.springframework.beans.BeanUtils.getPropertyDescriptors(fromClass);
    		
    		if (toPdArr != null && toPdArr.length != 0 && fromPdArr != null && fromPdArr.length != 0) {
    			for (PropertyDescriptor fromPd : fromPdArr) {
    				Method fromMethod = fromPd.getReadMethod();
					if (fromMethod != null) {
						Object val = fromMethod.invoke(from);
						
	    				//当值不为null则进行赋值
	    				if (val != null) {
	    					for (PropertyDescriptor toPd : toPdArr) {
	    						Method toMethod = toPd.getWriteMethod();
	    						if (toMethod != null && fromPd.getName().equals(toPd.getName())) {
	    							toMethod.invoke(to, val);
	    							break;
	    						}
	    					}
	    				}
			...
			throw new YgxcBusinessException("Copy real properties error", e);
		}
    }
}
